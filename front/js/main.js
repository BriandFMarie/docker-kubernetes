    ...
    $(".results").each(function() {
        $.ajax({
            ...
            success: function(data) {
                ...
                heightDog = (dog / (cat + dog) * 100) * 2;
                heightCat = (cat / (dog + cat) * 100) * 2;
                $(".result.cat").each(function() {
                    $(this).attr("data-height", heightCat).find("span.nbr").html((heightCat / 2) + "%");
                })
                $(".result.dog").each(function() {
                    $(this).attr("data-height", heightDog).find("span.nbr").html((heightDog / 2) + "%");
                })
